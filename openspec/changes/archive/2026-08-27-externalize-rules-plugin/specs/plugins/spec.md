...
系统 SHALL 提供 `PluginManager`，`install` / `installAll` 执行 plugin 的 `install`，把注入的能力收集进 `CapabilityBundle`（tools / skills / hooks / guardrails / promptFragments / memoryBackends / cacheBackends / vectorStores / embeddingProviders / tokenCounters / contextCompactors / retrievers / rerankers / summarizers / contextContributors，可含 dispose）。
...
系统 SHALL 提供 `assembleAgentLoop(options)`（async）：把 `CapabilityBundle` 的能力合并进 AgentLoop 的 sinks（tools 注册进 ToolRegistry、skills 合并、hooks 注册进 HookPipeline、prompt 片段注入 system prompt、contextContributors 注入循环），最终构造 `AgentLoop`。当传入 `security` 时，SHALL 调用 `registerBuiltinTools` 恒注册四个通用原语；当传入 `tools.disabled` 时，SHALL 在全部工具（builtin / plugin / mcp）注册完成后按名移除被禁用工具；todo 规划引导片段仅在 `builtin.todo` 最终仍在 registry 时注入。
