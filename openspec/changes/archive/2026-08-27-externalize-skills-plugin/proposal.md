...
本 change 把 skills 外放为 `@lhx-agent-engine/plugin-skills`，走 `ContextContributor` 统一缝（检索命中注入 instruction 文本 + 捆绑工具），core 删除 skill 硬路径与 `registerSkill` 能力轴。与 `plugin-rules` 完全对称。

## What Changes

- **新增 `@lhx-agent-engine/plugin-skills`**：`createSkillsPlugin(skills, options?)`（自建索引 + `hybridRetrieve` + `ContextContributor`）；迁入 `Skill` 类型、`loadSkillFromPath`、`resolveSkill`/`resolveSkills`/`createDefaultSkillSourceDeps`。
...
- **config 零迁移（D1-A）**：`config.skills` 字段不变，解释权移交 `@lhx-agent-engine/plugin-skills`。

## Capabilities

### New Capabilities

- `plugin-skills`: `@lhx-agent-engine/plugin-skills` 提供 `createSkillsPlugin`（检索 + 注入 + 工具）、`loadSkillFromPath`、`resolveSkill`/`resolveSkills`。
