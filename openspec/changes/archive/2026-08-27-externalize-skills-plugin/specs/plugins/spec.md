...
系统 SHALL 提供 `PluginManager`，`install` / `installAll` 执行 plugin 的 `install`，把注入的能力收集进 `CapabilityBundle`（tools / hooks / guardrails / promptFragments / memoryBackends / cacheBackends / vectorStores / embeddingProviders / tokenCounters / contextCompactors / retrievers / rerankers / summarizers / contextContributors，可含 dispose）。
