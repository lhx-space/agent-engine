...
- `read_file` / `write_file` 迁出内置，做成内置 plugin `@agent-engine/plugin-files`（经 `config.plugins` 声明加载）。
- `bash` 迁出内置，做成内置 plugin `@agent-engine/plugin-bash`（经 `config.plugins` 声明加载）。
...
- `plugins`: 新增内置 plugin `@agent-engine/plugin-files` / `@agent-engine/plugin-bash`。
...
- **Breaking**：`read_file`/`write_file`/`bash` 不再默认注册，需在 `config.plugins` 声明 `@agent-engine/plugin-files` / `@agent-engine/plugin-bash`；`json`/`base64`/`calculator`/`sitesearch` 移除。
